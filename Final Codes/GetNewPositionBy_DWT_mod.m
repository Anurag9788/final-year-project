function [ x1,y1,bplot ] =GetNewPositionBy_DWT_mod(handles,frame,x,y,H,W,T_Org,wavelet_type)
...
for i=up:stp:down-stp
    ...
    if (i < 1 || j < 1|| (i+H) > height || (j+W) > width)
          continue;
    end
      T123=frame(i:i+H-1,j:j+W-1,:);
        T=framegray(i:i+H-1,j:j+W-1);
        T=imnoise(T,'gaussian');
        Sdash=GetNextSubspaceFeatureVector_DWT_mod(T,wavelet_type);
        D=SSD(S,Sdash);
        if m==1
            min_D=D;
            dy=dyi;
